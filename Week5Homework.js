...
const num = data.map(user => {
  return user.phone;
});
console.log(num);

//create and print a list of website and email pairs
const webEmail = data.map(user => [user.email, user.website]);
...
const userTodo = dataTwo.filter(todo => todo.userid === 9);
console.log(userTodo);

//then find the number of user 9's incomplete tasks
const incompleteTasks = userTodo.filter(task => task.completed === false);
console.log(userTodo);
...
const userPost = dataThree.filter(title => title.userId === 5);
console.log(userPost);

//create a list of title and body pairs
const titleBody = dataThree.map(user => [user.title, user.body]);
